...
class Sports extends CI_Controller {
 ...
 public function index(){
        ...
        $data['forms']=$this->sports_model->GetAll();
       
        $data['page_title'] = 'List Sports Forms';
        $data['content_view'] = 'admin/sports/listform';
        $this->load->view('admin/template_new', $data);
 }
 
 public function approve_form($id){
      $form = $this->sports_model->GetById($id);
      
      $is_approved = $this->sports_model->Search(array('form_id'=>$form->form_id,'dept_id'=>$form->dept_id,'user_id'=>$form->user_id,'status'=>'approve'));
      if(empty($is_approved)){
        $data['form'] = $form;
        $data['content_view'] = 'admin/sports/approve-form';        
     ...
     public function show_approved_forms(){
        $forms = $this->sports_model->GetApprovedForms();
        $data['forms'] = $forms;
        $data['content_view'] = 'admin/sports/show-approved-forms';        
        $data['page_title'] = 'Approved Forms';
        $this->load->view('admin/template_new', $data); 
    }
      public function show_disapproved_forms(){
        $forms = $this->sports_model->GetDispprovedForms();
        $data['forms'] = $forms;
        $data['content_view'] = 'admin/sports/show-disapproved-forms';        
    ...
    public function disapprove_form($id){
      ...
      $is_disapproved = $this->sports_model->Search(array('form_id'=>$form->form_id,'dept_id'=>$form->dept_id,'user_id'=>$form->user_id,'status'=>'disapprove'));
      if(empty($is_disapproved)){
        $data['form'] = $form;
        $data['content_view'] = 'admin/sports/disapprove-form';        
  ...
  public function edit_form($id){
      $data['depts'] = $this->sports_model->GetAll();
      $data['edit_form']=$this->sports_model->GetById($id);
  ...
  public function add_form(){
    ...
    public function save_form(){
      ...
      if($this->form_validation->run()){
          ...
          $result=$this->sports_model->Save($data);
...
//        print_r($data);
//        exit();
        $is_approved = $this->sports_model->Search(array('form_id'=>$data['form_id'],'dept_id'=>$data['dept_id'],'user_id'=>$data['user_id'],'status'=>'approve'));
        if(empty($is_approved)){
//            echo "<pre>";
//            print_r($is_approved);
//            exit();
           $result=$this->sports_model->Save($data,$id);
            if($result)
             {
                ...
                $is_available = $this->final_form_model->Search(array('form_id'=>$data_final['form_id'],'dept_id'=>$data_final['dept_id'],'user_id'=>$data_final['user_id']));
                if(empty($is_available)){
                  $this->final_form_model->Save($data_final);  
                }
//                $this->final_form_model->Save($data_final);
...
//        print_r($data);
//        exit();
        $is_disapproved = $this->sports_model->Search(array('form_id'=>$data['form_id'],'dept_id'=>$data['dept_id'],'user_id'=>$data['user_id'],'status'=>'disapprove'));
        if(empty($is_disapproved)){
...
//                $is_exists = $this->final_form_model->Search(array('form_id'=>$data_lib['form_id'],'dept_id'=>$data_lib['dept_id'],'user_id'=>$data_lib['user_id']));
//                if(!empty($is_exists)){
//                    $is_exists = $is_exists[0];
//
//                    $this->final_form_model->DeleteForm($data_lib);
//                }

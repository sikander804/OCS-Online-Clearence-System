...
class Users_model extends CI_Model {
    ...
    {        
        $result = $this->db->get_where($this->table_name,array('full_name'=>$name))->row();
        return $result;
    }
    ...
    function getUsers($postData){
     ...
     if(isset($postData['search']) ){
       // Select record
       $this->db->select('*');
       $this->db->where("full_name like '%".$postData['search']."%' ");

       $records = $this->db->get('users')->result();

       foreach($records as $row ){
          $response[] = array("value"=>$row->id,"label"=>$row->full_name);
       }
